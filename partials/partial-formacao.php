        ...
        <div class="wrapper">
            <div class="header-item">
                <div class="background">
                    ...
                    <div class="grid-cont">
                        <div class="header-description" style="padding-bottom: 35px;">
                            <div class="square">
...
<div class="filters-holder js-filters margin-50">
    ...
    <div class="grid-cont">
        <div class="filter">
            <select id="inputCursos" data-exterior-label="<?php _e('Tipo', 'edit'); ?>"  name="inputCursos">
                ...
                foreach($tipos as $tipo):
                    ?>
                    <option value="<?php echo $tipo->ID ?>" <?php if($tipoId == $tipo->ID): ?>selected<?php endif; ?>><?php echo $tipo->post_title; ?></option>
                    <?php
                endforeach;
                ?>
            </select>
        ...
        <div class="filter">
            <select id="inputHorario" data-exterior-label="<?php dictionary("horario") ?>"  name="inputHorario">
                <option value="0" <?php if($horarioId == 0): ?>selected<?php endif; ?>>Todos</option>
                <?php
                $arguments = array(
                    'offset'           => '',
                    'showposts'        => '-1',
                    ...
                    'meta_key'         => '',
                    'meta_value'       => '',
                    'post_type'        => 'formacao_horarios',
                    'post_mime_type'   => '',
                    'post_parent'      => '',
                    'post_status'      => '',
                    'suppress_filters' => '0'
                );
                $horarios = get_posts( $arguments );
                foreach($horarios as $horario):
                    ?>
                    <option value="<?php echo $horario->ID ?>" <?php if($horarioId == $horario->ID): ?>selected<?php endif; ?>><?php echo $horario->post_title; ?></option>
                    <?php
                endforeach;
                ?>
            </select>
        ...
        <div class="filter">
            ...
            <select id="inputLocalizacao" data-exterior-label="<?php dictionary("Localizacao") ?>" name="inputLocalizacao">
                <option value="0" <?php if($localizacaoId == 0): ?>selected<?php endif; ?>>Todos</option>
                <?php
                $arguments = array(
                    'offset'           => '',
                    'showposts'        => '-1',
                ...
                foreach($localizacoes as $localizacao):?>
                    <option value="<?php echo $localizacao->ID ?>" <?php if($localizacaoId == $localizacao->ID): ?>selected<?php endif; ?>><?php echo $localizacao->post_title; ?></option>
                    <?php
                endforeach;
                ?>
            </select>
        ...
        <script>
            jQuery(document).ready(function( $ ) {
                Edit.modules.collection.push({ type: 'filters', instance: new Edit.modules.filtersModule('.js-filters', '/results/formacao-results/') })
            })
        </script>
    </div>
...
<div class="filtered-content">
    <div class="js-iso-module filtered margin-50 js-cursos">
        <div class="grid-cont">
            ...
            <?php get_template_part( 'partials/partial', 'formacao-result' ); ?>
        </div>
        <script>
            jQuery(document).ready(function( $ ) {
...
<script>
    jQuery(document).ready(function( $ ) {
        Edit.modules.collection.push({ type: 'loadMore', instance: new Edit.modules.loadMore('.js-loadmore', true, '/results/formacao-results/') });
    })
</script>
</div>
