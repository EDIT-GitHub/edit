<?php
/**
 * The template for Eventos
 *
 * @package Edit
 */

$inputLocalizacaoId = "0";
$inputAreaId = "0"; 


if(isset($_REQUEST['inputLocalizacao'])) {
    $inputLocalizacaoId = $_REQUEST['inputLocalizacao'];
}
...
<div class="grid-cont">
    <div class="filter">
        <select id="inputLocalizacao" data-exterior-label="<?php dictionary("Localizacao") ?>" name="inputLocalizacao">
           <option value="0" <?php if($inputLocalizacaoId == "0"): ?>selected<?php endif; ?>>Todos</option>
           <?php
           ...
           foreach($localizacoes as $localizacao):
            ?>
            <option value="<?php echo $localizacao->ID ?>" <?php if($inputLocalizacaoId == $localizacao->ID): ?>selected<?php endif; ?>><?php echo $localizacao->post_title; ?></option>
            <?php                                                                                                                                      
        endforeach;
        ?>
...
<script>
    jQuery(document).ready(function( $ ) {
        Edit.modules.collection.push({ type: 'filters', instance: new Edit.modules.filtersModule('.js-filters', '/results/eventos-results/') })
    })
</script>
</div>
...
<div class="filtered-content">
    <div class="js-iso-module filtered margin-50 js-cursos">
        <div class="grid-cont">
            ...
            <?php get_template_part( 'partials/partial', 'eventos-response' ); ?>
        </div>
        <script>
            jQuery(document).ready(function( $ ) {
...
<script>
    jQuery(document).ready(function( $ ) {
        Edit.modules.collection.push({ type: 'loadMore', instance: new Edit.modules.loadMore('.js-loadmore', true, '/results/eventos-results/‎') });
    })
</script>
</div>
